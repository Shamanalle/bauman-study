# Разбор кода: Лабораторная работа 2 (Вариант 16)
...
public:
    ...
    friend Vector operator^(const Vector& v, const int* arr);
};
...
Vector operator^(const Vector& v, const int* arr) {
    Vector result(v);
...
int main() {
    ...
    cout << "\n7. Операция ^ (побитовое XOR): Vector ^ int*\n";
...
### Задание: Побитовое XOR с массивом (`^`)
Правый операнд — `int*`.
