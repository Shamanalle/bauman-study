# Разбор кода: Лабораторная работа 5 (Вариант 26)
...
Если бы мы проверяли **только** поле `fuel[2]`:
- Два объекта с одинаковым значением `fuel[2]` считались бы эквивалентными.
- `!(fuel[2]_a < fuel[2]_b)` и `!(fuel[2]_b < fuel[2]_a)` — оба ложные, значит дерево решит, что это **одна и та же** запись и выбросит вторую.

Мы избегаем этого, добавляя дополнительный критерий `model` в случае равных значений основного поля (составное/каскадное условие сортировки).
...
```
Обход дерева `set` через range-based `for` гарантированно выдаёт элементы в **отсортированном порядке** (по смешанному расходу топлива).
