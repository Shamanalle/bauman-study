# Разбор кода: Лабораторная работа 2 (Вариант 26)
...
### Задание: Умножение вектора на массив (`*`)
```cpp
Vector operator*(const Vector& v, const double* arr) {
    Vector result(v);
    for (int i = 0; i < v.size(); i++)
...
```
Правый операнд — обычный массив `double*`. Функция поэлементно умножает вектор на массив.
