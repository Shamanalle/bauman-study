# Разбор кода: Лабораторная работа 4 (Вариант 9)
...
int main() {
    ...
    sort(items.begin(), items.end(), compareBySalary);

    fout << "\nОтсортированный контейнер (по окладу):\n";
    cout << "\nОтсортированный контейнер (по окладу):\n";
    for (const auto& item : items) {
        fout << item << "\n";
        cout << item << "\n";
    ...
    deque<Employee> copied(items.size());
...
```cpp
    Employee() : name(""), hireDate(""), position(""), salary(0.0) {}
```
**Конструктор по умолчанию** необходим для работы с STL-контейнерами. Когда мы пишем `deque<Employee> copied(items.size())`, контейнер создает `items.size()` объектов, вызывая для каждого конструктор по умолчанию.
...
sort(items.begin(), items.end(), compareBySalary);
```
Стандартная библиотека `<algorithm>` позволяет очень быстро отсортировать контейнер. Для этого мы передаем `begin()` (итератор на начало), `end()` (знак "за концом" контейнера) и свою функцию-компаратор, которая объясняет процедуре, какое именно значение нужно считать "меньшим" (сравниваем по окладу).
