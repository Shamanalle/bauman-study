# Разбор кода: Лабораторная работа 5 (Вариант 15)
...
Если бы мы проверяли **только** поле `title`:
- Два объекта с одинаковым значением `title` считались бы эквивалентными.
- `!(title_a < title_b)` и `!(title_b < title_a)` — оба ложные, значит дерево решит, что это **одна и та же** запись и выбросит вторую.

Мы избегаем этого, добавляя дополнительный критерий `author` в случае равных значений основного поля (составное/каскадное условие сортировки).
