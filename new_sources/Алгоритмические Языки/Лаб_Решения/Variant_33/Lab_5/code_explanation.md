# Разбор кода: Лабораторная работа 5 (Вариант 33)
...
Если бы мы проверяли **только** поле `salary`:
- Два объекта с одинаковым значением `salary` считались бы эквивалентными.
- `!(salary_a < salary_b)` и `!(salary_b < salary_a)` — оба ложные, значит дерево решит, что это **одна и та же** запись и выбросит вторую.
...
```
Обход дерева `set` через range-based `for` гарантированно выдаёт элементы в **отсортированном порядке** (по окладу).
