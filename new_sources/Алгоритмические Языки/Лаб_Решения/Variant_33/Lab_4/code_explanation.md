# Разбор кода: Лабораторная работа 4 (Вариант 33)
...
```cpp
    Employee() : name(""), hireDate(""), position(""), salary(0.0) {}
```
**Конструктор по умолчанию** необходим для работы с STL-контейнерами. Когда мы пишем `list<Employee> copied(items.size())`, контейнер создает `items.size()` объектов, вызывая для каждого конструктор по умолчанию.
