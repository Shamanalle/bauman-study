# Разбор кода: Лабораторная работа 2 (Вариант 21)
...
public:
    ...
    Vector(const bool* arr, int size) {
        n = size;
        p = new bool[n];
        for (int i = 0; i < n; i++) {
            p[i] = arr[i];
    ...
    friend Vector operator^(const bool* arr, const Vector& v);
};
...
Vector operator^(const bool* arr, const Vector& v) {
    Vector result(v); 
...
int main() {
    ...
    cout << "\n7. Вариант 21: операция ^\n";
    bool arr_op[] = {true, true, false, false, true}; 
    cout << "Обычный массив arr_op: 5 | 1 1 0 0 1" << endl;
    cout << "Операнд Vector v2:     " << v2 << endl;
    
    Vector v6 = arr_op ^ v2; 
    cout << "Результат (arr_op ^ v2): " << v6 << endl;
...
Поскольку класс `Vector` хранит указатель `bool* p`, память под который выделяется динамически через `new`, мы обязаны вручную реализовывать 5 специальных методов для управления этой памятью.
...
### Оператор `[]`
```cpp
    bool& operator[](int index) { return p[index]; }
```
...
### Задание по варианту 21: Логическое XOR (`^`)
```cpp
Vector operator^(const bool* arr, const Vector& v) {
    Vector result(v);
    for (int i = 0; i < v.size(); i++)
        result[i] = (arr[i] != v[i]);  // Логическое XOR
    return result;
}
```
Левый операнд — `bool*`, не `Vector`. Функция **обязательно** определяется вне класса (как `friend`). Логическое XOR для `bool` эквивалентно `!=`.
