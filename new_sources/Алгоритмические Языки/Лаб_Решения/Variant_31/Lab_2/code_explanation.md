# Разбор кода: Лабораторная работа 2 (Вариант 31)
...
int main() {
    ...
    cout << "\n7. Операция -- (постфиксный декремент)\n";
    cout << "v2 до декремента: " << v2 << endl;
    Vector v6 = v2--;
    cout << "Результат v2--:   " << v6 << " (старый)" << endl;
    cout << "v2 после --:      " << v2 << endl;
...
Поскольку класс `Vector` хранит указатель `int* p`, память под который выделяется динамически через `new`, мы обязаны вручную реализовывать 5 специальных методов для управления этой памятью.
...
### Задание: Постфиксный декремент (`--`)
```cpp
Vector operator--(Vector& v, int) {  // int — фиктивный параметр для постфиксной формы
    Vector old(v);  // сохраняем копию
    for (int i = 0; i < v.size(); i++) v[i]--;
    return old;     // возвращаем старое значение
}
```
Фиктивный параметр `int` отличает постфиксную форму от префиксной. Результат — копия вектора **до** изменения.
