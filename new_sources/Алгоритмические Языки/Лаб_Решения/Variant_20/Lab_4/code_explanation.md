# Разбор кода: Лабораторная работа 4 (Вариант 20)
...
items.sort(compareByMixedFuel);
```
Контейнер `list` предоставляет только двунаправленные итераторы (Bidirectional Iterator), а для алгоритма `std::sort` нужны итераторы произвольного доступа (Random Access Iterator). Поэтому у `list` есть **собственный метод** `sort()`, который принимает функцию-компаратор. Компаратор объясняет, какой элемент считать «меньшим» — сортировка по смешанному расходу топлива.
