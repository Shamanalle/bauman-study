# Разбор кода: Лабораторная работа 2 (Вариант 20)
...
public:
    ...
    friend Vector operator^(const Vector& v, const bool* arr);
};
...
Vector operator^(const Vector& v, const bool* arr) {
    Vector result(v);
...
int main() {
    ...
    cout << "\n7. Операция ^ (логическое XOR): Vector ^ bool*\n";
    bool arr_op[] = {true, true, false, false, true};
    Vector v6 = v2 ^ arr_op;
    cout << "v2 ^ arr_op = " << v6 << endl;
...
### Задание: Логическое XOR с массивом (`^`)
Правый операнд — `bool*`.
