# Разбор кода: Лабораторная работа 2 (Вариант 13)
...
public:
    ...
    friend double operator*(const double* arr, const Vector& v);
};
...
double operator*(const double* arr, const Vector& v) {
    double result = 0;
...
int main() {
    ...
    cout << "\n7. Операция * (скалярное произведение): double* * Vector\n";
    double arr_op[] = {2.0, 3.0, 1.0, 0.5, 4.0};
    double dotProduct = arr_op * v2;
    cout << "arr_op * v2 = " << dotProduct << endl;
...
### Задание: Скалярное произведение (`*`)
Левый операнд — `double*`. Функция **обязательно** вне класса. Результат — `double`.
