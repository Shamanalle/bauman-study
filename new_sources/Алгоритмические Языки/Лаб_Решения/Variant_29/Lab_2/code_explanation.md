# Разбор кода: Лабораторная работа 2 (Вариант 29)
...
public:
    ...
    friend Vector operator+(const Vector& v, const double* arr);
};
...
Vector operator+(const Vector& v, const double* arr) {
    int newN = v.size() * 2;
    double* tmp = new double[newN];
    for (int i = 0; i < v.size(); i++) tmp[i] = v[i];
    for (int i = 0; i < v.size(); i++) tmp[v.size() + i] = arr[i];
...
int main() {
    ...
    cout << "\n7. Операция + (конкатенация): Vector + double*\n";
...
### Задание: Конкатенация вектора и массива (`+`)
Левый операнд — `Vector`, правый — `double*`. Длина массива считается равной размеру вектора.
