# Разбор кода: Лабораторная работа 6 (Вариант 25)
...
private:
    vector<double> err_arr;
    double err_first, err_ratio;
```
Наследник `invalid_argument`. Хранит копию массива, первый элемент и знаменатель.

## 2. Класс GeomProgression и `throw`
```cpp
    bool isValid(const double* arr, int n) const {{
        if (arr[0] != first) return false;
        for (int i = 1; i < n; i++) {{
...
```
Проверка: первый элемент = `first`, каждый следующий = предыдущий × `ratio`.

```cpp
    GeomProgression(double f, double r, const double* arr, int n) {{
        if (!isValid(arr, n)) throw GeomException("Сообщение!", arr, n, f, r);
        // память выделяется только после проверки
    }}
```
...
```cpp
    try {{
        double badArr[] = {{3.0, 6.0, 11.0, 24.0}};  // 11.0 != 6.0*2=12.0
        GeomProgression gp2(3.0, 2.0, badArr, 4);
    }}
    catch (const GeomException& e) {{
...
```
Порядок: `GeomException` → `invalid_argument` → `exception`.
