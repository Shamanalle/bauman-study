# Разбор кода: Лабораторная работа 2 (Вариант 25)
...
```
Оба операнда — объекты `Vector`, поэтому функция может быть определена как дружественная. Создаем копию первого вектора и поэлементно прибавляем второй.
