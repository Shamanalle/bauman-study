# Разбор кода: Лабораторная работа 5 (Вариант 35)
...
int main() {
    ...
    cout << "Дерево set (Сортировка по сумме кредита):\n";
...
Если бы мы проверяли **только** поле `amount`:
- Два объекта с одинаковым значением `amount` считались бы эквивалентными.
- `!(amount_a < amount_b)` и `!(amount_b < amount_a)` — оба ложные, значит дерево решит, что это **одна и та же** запись и выбросит вторую.
...
```
Обход дерева `set` через range-based `for` гарантированно выдаёт элементы в **отсортированном порядке** (по сумме кредита).
