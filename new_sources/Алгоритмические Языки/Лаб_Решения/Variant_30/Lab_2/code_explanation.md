# Разбор кода: Лабораторная работа 2 (Вариант 30)
...
public:
    ...
    friend Vector operator+(const double* arr, const Vector& v);
};
...
Vector operator+(const double* arr, const Vector& v) {
    int newN = v.size() * 2;
    double* tmp = new double[newN];
    for (int i = 0; i < v.size(); i++) tmp[i] = arr[i];
    for (int i = 0; i < v.size(); i++) tmp[v.size() + i] = v[i];
    Vector result(tmp, newN);
    delete[] tmp;
    return result;
}
...
int main() {
    ...
    cout << "\n7. Операция + (конкатенация): double* + Vector\n";
    double arr_op[] = {10.0, 20.0, 30.0, 40.0, 50.0};
    Vector v6 = arr_op + v2;
...
### Задание: Конкатенация массива и вектора (`+`)
Левый операнд — `double*`. Функция **обязательно** определяется вне класса (как `friend`), т.к. левый операнд — не Vector.
