# Разбор кода: Лабораторная работа 4 (Вариант 28)
...
```cpp
    Book() : author(""), title(""), publisher(""), year(0), pages(0) {}
```
**Конструктор по умолчанию** необходим для работы с STL-контейнерами. Когда мы пишем `vector<Book> copied(items.size())`, контейнер создает `items.size()` объектов, вызывая для каждого конструктор по умолчанию.
...
items.sort(compareByAuthor);
```
Контейнер `list` предоставляет только двунаправленные итераторы (Bidirectional Iterator), а для алгоритма `std::sort` нужны итераторы произвольного доступа (Random Access Iterator). Поэтому у `list` есть **собственный метод** `sort()`, который принимает функцию-компаратор. Компаратор объясняет, какой элемент считать «меньшим» — сортировка по ФИО автора.
