# Разбор кода: Лабораторная работа 2 (Вариант 28)
...
Vector operator+(const Vector& v1, const Vector& v2) {
    int newN = v1.size() + v2.size();
    double* tmp = new double[newN];
    for (int i = 0; i < v1.size(); i++) tmp[i] = v1[i];
    for (int i = 0; i < v2.size(); i++) tmp[v1.size() + i] = v2[i];
...
int main() {
    ...
    cout << "\n7. Операция + (конкатенация): Vector + Vector\n";
    Vector v6 = v1 + v2;
    cout << "v1 + v2 = " << v6 << endl;
...
```
Создаётся временный массив суммарного размера, заполняется элементами обоих векторов, затем конструируется результат.
