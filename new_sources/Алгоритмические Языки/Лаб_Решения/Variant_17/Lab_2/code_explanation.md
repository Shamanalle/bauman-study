# Разбор кода: Лабораторная работа 2 (Вариант 17)
...
public:
    ...
    friend Vector operator^(const int* arr, const Vector& v);
};
...
Vector operator^(const int* arr, const Vector& v) {
    Vector result(v);
...
int main() {
    ...
    cout << "\n7. Операция ^ (побитовое XOR): int* ^ Vector\n";
    int arr_op[] = {0xFF, 0x0F, 0xAA, 0x55, 0x00};
    Vector v6 = arr_op ^ v2;
    cout << "arr_op ^ v2 = " << v6 << endl;
...
### Задание: Побитовое XOR массива и вектора (`^`)
