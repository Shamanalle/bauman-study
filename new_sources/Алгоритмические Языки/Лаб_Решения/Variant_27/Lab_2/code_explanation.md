# Разбор кода: Лабораторная работа 2 (Вариант 27)
...
### Задание: Умножение массива на вектор (`*`)
```cpp
Vector operator*(const double* arr, const Vector& v) { ... }
```
Левый операнд — `double*`, не `Vector`. Поэтому функция **обязательно** определяется вне класса (как `friend`).
