# Разбор кода: Лабораторная работа 2 (Вариант 24)
...
public:
    ...
    friend bool operator<(const int* arr, const Vector& v);
};
...
bool operator<(const int* arr, const Vector& v) {
...
int main() {
    ...
    cout << "\n7. Операция < (сравнение): int* < Vector\n";
    int arr_op[] = {1, 2, 3, 0, 10};
    bool result = (arr_op < v2);
    cout << "arr_op < v2 = " << (result ? "true" : "false") << endl;
...
### Задание: Сравнение массива с вектором (`<`)
Левый операнд — `int*`. Функция **обязательно** вне класса (как `friend`).
