# Разбор кода: Лабораторная работа 4 (Вариант 23)
...
```cpp
    BankLoan() : name(""), amount(0.0), currency("RUB"), interestRate(0.0) {}
```
**Конструктор по умолчанию** необходим для работы с STL-контейнерами. Когда мы пишем `deque<BankLoan> copied(items.size())`, контейнер создает `items.size()` объектов, вызывая для каждого конструктор по умолчанию.
