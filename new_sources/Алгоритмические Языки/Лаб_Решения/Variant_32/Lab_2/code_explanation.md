# Разбор кода: Лабораторная работа 2 (Вариант 32)
...
int main() {
    ...
    Vector v6 = v2 * multiplier;
    cout << "v2 * " << multiplier << " = " << v6 << endl;
...
### Задание: Умножение вектора на число (`*`)
```cpp
Vector operator*(const Vector& v, double x) {
    Vector result(v);
    for (int i = 0; i < v.size(); i++)
...
```
Правый операнд — скаляр `double`. Каждый элемент вектора умножается на это число.
