# Разбор кода: Лабораторная работа 4 (Вариант 32)
...
```cpp
    BankDeposit() : name(""), amount(0.0), currency("RUB"), rate(0.0) {}
```
**Конструктор по умолчанию** необходим для работы с STL-контейнерами. Когда мы пишем `list<BankDeposit> copied(items.size())`, контейнер создает `items.size()` объектов, вызывая для каждого конструктор по умолчанию.
